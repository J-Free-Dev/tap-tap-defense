...
# Game state
@export var max_health: int = 3  # Maximum lives/health
var current_health: int = 3  # Current lives
...
func _on_enemy_killed(points_value: int):
...
func _on_enemy_escaped():
	# Reduce health when enemy escapes
	current_health -= 1
	health_changed.emit(current_health)
	...
	if current_health <= 0:
		...
		# Check if wave is complete (even if enemy escaped)
		check_wave_complete()
